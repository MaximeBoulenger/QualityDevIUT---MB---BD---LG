...
public class Library
{
    ...
    public bool BorrowMedia(string title, string user)
    {
        ...
        if (media != null)
        {
            media.AvailableCopies--; // Decrease the available copies
    ...
    public bool ReturnMedia(string title, string user)
    {
        ...
        if (borrow != null)
        {
            borrow.Media.AvailableCopies++; // Increase the available copies
    ...
    public List<Media> SearchMedia(string searchCriteria)
    {
        return mediaCollection.Where(m =>
            ...
            (m is Book && ((Book)m).Author.Contains(searchCriteria, StringComparison.OrdinalIgnoreCase))
        ).ToList();
    }
